FD Saathi 2.0 – FD Retention & Loan Conversion Engine


# Problem

Many users break their Fixed Deposits during emergencies.
This leads to unnecessary financial loss due to penalties, even when better options exist.
---


#  Solution

FD Saathi is an intelligent financial decision system that compares FD breakage vs loan cost in real time.
It helps users choose the most cost-effective option and prevents unnecessary loss.
---

# Key Features

* AI-based decision engine for financial comparison
* FD vs loan cost analysis in real time
* Automatic savings calculation
* WhatsApp-style user interaction simulation
* Simple explanations for senior citizens
* Revenue impact dashboard for fintech companies
* ---

## 🎥 Demo Video ....... FOR  DEMO VIDEO 
[FD Saathi Demo video](https://drive.google.com/file/d/1v388AIK2ksj9v-D6n9jTwlxBPVWlYdrD/view?usp=sharing)
---

## 🌐 Live Demo FOR LIVE DEMO 

 [FD Saathi Live Demo](https://fdsaathifintech.netlify.app)
---

## 🧠 System Flow

User input → FD data analysis → Decision engine → Loan vs FD comparison → Final recommendation

---

# Business Impact

* Reduces FD breakage across users
* Helps banks retain Assets Under Management (AUM)
* Generates additional revenue through loan conversions
* Improves customer financial decision-making

---

# Tech Stack

Frontend: HTML, Tailwind CSS, JavaScript
Backend (simulation): FastAPI / Node.js logic
AI Layer: Decision-based financial engine

---

# How to Run

1. Download or clone the project
2. Open index.html in any browser
3. Or deploy using Netlify for live demo

---



